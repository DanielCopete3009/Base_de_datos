...
SELECT * FROM jugadores WHERE nombre = 'Aleix García';
DELETE FROM jugadores WHERE nombre = 'Aleix García';

-- 2. Borrar goles de un partido
SELECT * FROM goles WHERE partido_id = 3;
DELETE FROM goles WHERE partido_id = 3;

-- 3. Eliminar goles después del minuto 85
SELECT * FROM goles WHERE minuto > 85;
DELETE FROM goles WHERE minuto > 85;

-- 4. Eliminar partidos sin goles
SELECT * FROM partidos WHERE goles_casa = 0 AND goles_fuera = 0;
DELETE FROM partidos WHERE goles_casa = 0 AND goles_fuera = 0;

-- 5. Borrar jugadores sin equipo
SELECT * FROM jugadores WHERE equipo_id IS NULL;
DELETE FROM jugadores WHERE equipo_id IS NULL;

-- 6. Eliminar goles de Gerard Moreno
SELECT g.*
FROM goles g
JOIN jugadores j ON g.jugador_id = j.id
WHERE j.nombre = 'Gerard Moreno';
...
SELECT * FROM goles ORDER BY id DESC LIMIT 2;
DELETE FROM goles ORDER BY id DESC LIMIT 2;

-- 8. Borrar partidos antes del 1 de abril de 2024
SELECT * FROM partidos WHERE fecha < '2024-04-01';
DELETE FROM partidos WHERE fecha < '2024-04-01';

-- 9. Eliminar goles del partido 5 antes del minuto 30
SELECT * FROM goles WHERE partido_id = 5 AND minuto < 30;
DELETE FROM goles WHERE partido_id = 5 AND minuto < 30;

-- 10. Eliminar equipo Girona Futbol Club si no tiene jugadores
SELECT * FROM equipos
WHERE nombre = 'Girona Futbol Club'
AND id NOT IN (SELECT equipo_id FROM jugadores);
